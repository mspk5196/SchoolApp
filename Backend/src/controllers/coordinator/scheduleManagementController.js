        ...
        try {
            ...
            for (let i = 0; i < rows.length; i++) {
             ...
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?)`,
                        [
                            date,
                            `${isAssessment === 1 ? "Assessment" : "Academic"} - Batch ${cleanBatchLevel}`,
                            isAssessment === 1 ? "Assessment" : "Academic",
                            sectionId,
                            duration,
                            batchId?.id,
                            mentor?.id || null,
...
const createStudentWiseSchedule = async (sectionId, days, includeToday) => {
    ...
    try {
        ...
        for (const activity of periodActivities) {
           ...
           console.log('Prepared activity data for insertion:', activityData);
