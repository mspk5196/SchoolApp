...
const AdminCoordinatorlistDetails = ({ route, navigation }) => {
  const { coordinator, roles, grades } = route.params;
  ...
  const [mentor, setMentor] = useState(coordinator);

  useEffect(() => {
    console.log(coordinator, roles, grades);

    roles.includes('Mentor') ? setIsMentor(true) : setIsMentor(false);
    if (isMentor) {
      console.log('Mentor');
      fetchMentorId();
    }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        />
        <Text style={styles.headerTxt}>Coordinator Details</Text>
      </View>

      <View style={styles.MentorDayDetails}>
        <View style={styles.profileSection}>
          {coordinator.file_path ? (
            <Image source={getProfileImageSource(coordinator.file_path)} style={styles.avatar} />
          ) : (
            <Image source={Staff} style={styles.avatar} />
          )}
          <View style={styles.mentorInfo}>
            <Text style={styles.infoLabel}>Name : {coordinator.name || 'N/A'}</Text>
            <Text style={styles.infoLabel}>Mentor ID : {coordinator.roll || 'N/A'}</Text>
          </View>
        </View>
      ...
      <View style={styles.infoBox}>
        <View style={styles.infoRow}>
          ...
          {isMentor && roles.includes('Coordinator') ? (
            <View style={{ flexDirection: 'row', flex: 3, gap: 8 }}>
              <View style={[styles.infoColumn, { flexWrap: 'wrap', justifyContent: 'center' }]}>
                <Text style={styles.infoTitle1}>Mentor For:</Text>
                <Text style={[styles.infoValue, { textAlign: 'center' }]}>{'Section ' + mentorDetails.section || 'NA'}</Text>
              </View>
              <View style={[styles.infoColumn, { flexWrap: 'wrap', justifyContent: 'center', alignItems: 'center' }]}>
                <Text style={[styles.infoTitle1]}>Coordinator For:</Text>
                <Text style={[styles.infoValue]}>{`Grade ${grades}`}</Text>
              </View>
            </View>
          ) : !isMentor && roles.includes('Coordinator') ? (
            <View style={styles.infoColumn}>
              <Text style={[styles.infoTitle1, { width: 90, textAlign: 'left' }]}>Coordinator For:</Text>
              <Text style={[styles.infoValue, { width: 40, textAlign: 'center', flexWrap: 'wrap' }]}>{`Grade ${grades}`}</Text>
            </View>
          ) : (
            <View style={styles.infoColumn}>
              <Text style={styles.infoTitle1}>Mentor For:</Text>
...
export default AdminCoordinatorlistDetails;